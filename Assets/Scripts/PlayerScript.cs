...
public class PlayerScript : MonoBehaviour
{
    ...
    IEnumerator BlockDestroyedCoroutine2()
    {
        for(int i = 0; i < 10; i++)
        {
            ...
            audioSrc.PlayOneShot(pointSound, 6);
        }
    }
    ...
    IEnumerator BallDestroyedCoroutine()
    {
        ...
        if (GameObject.FindGameObjectsWithTag("Ball").Length == 0)
            ...
            {
                gameData.Reset();
                SceneManager.LoadScene("SampleScene");//поенял сцену
            }
    }
    ...
    void StartLevel()
    {
        ...
        var xMax = Camera.main.orthographicSize * 0.8f * Camera.main.aspect * 0.85f;
    ...
    public void Resume()
    {
        PanelPause.SetActive(false); // Скрыть экран паузы
        Time.timeScale = 1;
        isPaused = false;
        Cursor.visible = false;
    }
